...
; d = map selected on the Town Map. D survives callfar/Bankswitch.
	...
	call ClearSprites
	call ClearScreen
	; SET_PAL_GENERIC loads PAL_REDBAR for caught-ball tile
...
.exit
; Restore the overworld encounter data. The Town Map caller reloads its own
; graphics/OAM from scratch, so this screen never restores PureRGB buffers.
	callfar LoadWildData
	ret

.noWildData
; Safety fallback. The Town Map caller normally filters these locations before
; entering the viewer.
	jp LoadCurrentMapWildDataFar
