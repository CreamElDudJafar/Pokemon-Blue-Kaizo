...
.inputLoop
	...
	ret
.pressedA
; Preserve the exact highlighted Town Map entry. This is the cursor/scroll
; location the player was viewing before opening wild encounter data.
	ld a, [wWhichTownMapLocation]
	push af
; Check for encounter data before opening the viewer. If the location has no
; Grass, Surf, or Rod encounters, pressing A leaves the Town Map alone.
...
.hasWildData
	...
	pop af
	ld [wWhichTownMapLocation], a
; Reload the Town Map through its normal engine path after a viewer was shown.
	call LoadTownMap
	ld a, [wCurMap]
	ld b, $0
	call DrawPlayerOrBirdSprite
